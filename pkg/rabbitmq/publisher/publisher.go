...
func (p *Publisher) connectionState() {
	...
	defer p.logger.Debug("[DEBUG] publisher stopped")

	p.logger.Debug("[DEBUG] publisher starting")
	...
	for {
		...
		case conn, ok := <-p.connCh:
			...
			if err != nil {
				p.logger.Debug("[DEBUG] publisher unready")
...
func (p *Publisher) channelState(conn AMQPConnection, connCloseCh <-chan struct{}) error {
	for {
		...
		if err != nil {
			p.logger.Error("[ERROR] init func: %s", err)
...
func (p *Publisher) handleConfirmations(
	...
	case state := <-p.internalStateCh:
		if state.Unready != nil {
			p.logger.Warn("[WARN] handle confirmation unexpected unready")
			return
		}

		p.logger.Debug("[DEBUG] handle confirmation ready")
	case <-confirmationCloseCh:
		return
	}

	p.logger.Debug("[DEBUG] handle confirmation started")
	defer p.logger.Debug("[DEBUG] handle confirmation stopped")
...
func (p *Publisher) publishState(ch AMQPChannel, connCloseCh <-chan struct{}, resultChCh chan chan error) error {
	...
	p.logger.Debug("[DEBUG] publisher ready")
	state := p.notifyReady()
	for {
		select {
		case p.internalStateCh <- state:
		...
		case <-chCloseCh:
			p.logger.Debug("[DEBUG] channel closed")
...
func (p *Publisher) pausedState(chFlowCh <-chan bool, connCloseCh <-chan struct{}, chCloseCh chan *amqp.Error) error {
	p.logger.Warn("[WARN] publisher flow paused")
	...
	for {
		...
		case resume := <-chFlowCh:
			if resume {
				p.logger.Info("[INFO] publisher flow resumed")
				return nil
			}
		case <-chCloseCh:
			p.logger.Debug("[DEBUG] channel closed")
...
func (p *Publisher) close(ch AMQPChannel) {
	if ch != nil {
		if err := ch.Close(); err != nil && !strings.Contains(err.Error(), "channel/connection is not open") {
			p.logger.Error("[WARN] publisher: channel close: %s", err)
		}
	}
}
