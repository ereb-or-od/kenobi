...
import (
	...
	"github.com/ereb-or-od/kenobi/pkg/rabbitmq/publisher"
	"sync"
	"time"
...
func NewDialer(opts ...Option) (*Dialer, error) {
	if defaultLogger, err := logging.New(); err != nil {
		return nil, err
	} else {
		c := &Dialer{
			config: config{
...
func (c *Dialer) connectState() {
	...
	defer c.cancelFunc()
	defer c.logger.Debug("[DEBUG] connection closed")

	i := 0
	l := len(c.amqpUrls)

	c.logger.Debug("[DEBUG] connection unready")
		...
		for {
			...
			case conn := <-connCh:
				...
				if err := c.connectedState(conn); err != nil {
					c.logger.Error("[ERROR] connection unready: %s", err)
				...
				return
			case err := <-errorCh:
				c.logger.Error("[ERROR] connection unready: %v", err)
...
func (c *Dialer) connectedState(amqpConn AMQPConnection) error {
	...
	c.logger.Debug("[DEBUG] connection ready")
	state := c.notifyReady()
	for {
		select {
		case c.internalStateChan <- state:
